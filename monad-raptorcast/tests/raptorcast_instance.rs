...
async fn test_priority_messages() {
    ...
    let known_addresses =
        HashMap::from([(tx_nodeid, tx_addr.clone()), (rx_nodeid, rx_addr.clone())]);
    ...
    let epoch = Epoch(0);
    let validator_set = vec![(tx_nodeid, Stake(1)), (rx_nodeid, Stake(1))];
    ...
    for i in 0..MESSAGE_COUNT {
        ...
        tx_rc.exec(vec![RouterCommand::PublishWithPriority {
            ...
            priority: monad_raptorcast::UdpPriority::High,
        }]);
    }
    ...
    for i in 0..MESSAGE_COUNT {
        ...
        tx_rc.exec(vec![RouterCommand::PublishWithPriority {
            ...
            priority: monad_raptorcast::UdpPriority::Regular,
        }]);
    }
...
async fn test_raptorcast_forwarding_priority() {
    ...
    let known_addresses = HashMap::from([
        (validator1_nodeid, validator1_addr.clone()),
        (validator2_nodeid, validator2_addr.clone()),
        (validator_fullnode_nodeid, validator_fullnode_addr.clone()),
    ]);
    ...
    let validator_set = vec![
        (validator1_nodeid, Stake(1)),
        (validator2_nodeid, Stake(1)),
        (validator_fullnode_nodeid, Stake(1)),
    ];
    ...
    validator2_rc.exec(vec![RouterCommand::UpdateFullNodes(vec![
