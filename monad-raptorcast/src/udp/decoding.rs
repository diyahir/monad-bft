...
type MessageKey<PK> = (NodeId<PK>, AppMessageHash);
type Cache<PK> = quick_cache::Cache<MessageKey<PK>, DecoderState>;

// Cache weight = clamp(static weight + dynamic weight, 1, 1000)
//
// Static weight:
//
// - base: +500
...
// - broadcast: +100
// - total symbols: -N
// - num of author messages: -2^(max(N-10,0))
//   (example: N=0,5,10,15,20; weight=0,0,-1,-32,-1024)
//
// Dynamic weight:
//
// - received symbols: +N
// - age: (seconds since first message) * -10
//
//

type ValidatorSet<PK> = BTreeMap<NodeId<PK>, Validator>;

trait PriorityPolicy {
    // weight the factors to determine the priority of a message
    // messages with larger priority is more likely to be evicted.
    fn priority(
        &self,
        from_validator: bool, // used to prioritize validator messages
        received_symbols: usize, // used to decide progress
        total_symbols: usize, // related to the size of the message
        author_msg_count: usize, // number of pending messages per author
        age: Duration, // how long has this message been pending
    ) -> u64;
}

struct PriorityCache<PK>
where
    PK: PubKey,
{
    author_bag: HashMap<NodeId<PK>, usize>,
    messages: HashMap<MessageKey<PK>, DecoderState>,
}

#[derive(Debug, Clone)]
pub(crate) struct DecoderCacheConfig {
    max_recently_decoded_symbols_retained: NonZero<usize>,
}
...
where
    PK: PubKey,
{
    pending_messages: QuickCache<>,
    recently_decoded: LruCache<AppMessageHash, RecentlyDecodedState>,
}

impl<PP, PK> Default for DecoderCache<PP, PK>
where
    PK: PubKey,
    PP: PriorityPolicy,
{
    ...
    PK: PubKey,
{
    pub fn new(config: DecoderCacheConfig) -> Self {
        Self {
            ...
            pending_messages: PriorityCache::default(),
        }
    }
