    ...
    ST: CertificateSignatureRecoverable,
{
    ...
    pub fn iter_skip_self_and_sender(
        ...
        let sender_id_ix = if let Some(root_vid) = self.validator_id {
            ...
            if maybe_pos_sender_id.is_err() {
                warn!("Author {} is not a member of raptorcast group", sender_id);
                return self.empty_iterator();
            }
    ...
    pub fn check_sender_node_id(&self, sender_id: &NodeId<CertificateSignaturePubKey<ST>>) -> bool {
        if let Some(root_vid) = self.validator_id {
            ...
            if !good {
                debug!(?sender_id, ?root_vid, "check_author_node_id (fn) failed");
            }
            good
        } else {
            ...
            if !good {
                debug!(?sender_id, ?self.sorted_other_peers, "check_author_node_id (v2v) failed");
            }
            good
        }
    }
    ...
    ST: CertificateSignatureRecoverable,
{
        ...
        match broadcast_mode {
            BroadcastMode::Primary => {
                ...
                if let Some(group) = self.validator_map.get(&msg_epoch) {
                    ...
                    if !sender_found {
                        debug!(?sender_node_id, ?self.validator_map,
            ...
            BroadcastMode::Secondary => {
                ...
                if !sender_found {
                    debug!(?sender_node_id, ?self.fullnode_map,
    ...
    pub fn check_round(
        ...
        if let Some(group_queue) = self.fullnode_map.get(sender_node_id) {
            ...
            // which is the normal case for BroadcastMode::Primary
            true
        }
    }
    ...
    pub fn iterate_rebroadcast_peers(
        ...
        let maybe_group = match broadcast_mode {
            ...
            BroadcastMode::Secondary => {
                let maybe_group_queue = self.fullnode_map.get(msg_sender);
                if let Some(group_queue) = maybe_group_queue {
                    group_queue.peek() // Take earliest among all future groups for msg_sender
                } else {
                    None
                }
            }
