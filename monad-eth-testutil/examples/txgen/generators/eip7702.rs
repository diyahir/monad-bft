...
impl EIP7702Generator {
    pub fn new(batch_call_contract: EIP7702, tx_per_sender: usize) -> Self {
        Self {
            batch_call_contract,
            tx_per_sender,
...
impl Generator for EIP7702Generator {
    fn handle_acct_group(
        ...
        while self.authorizations.len() < 5 && !accts.is_empty() {
        ...
        for sender in accts {
            for _ in 0..self.tx_per_sender {
                if !self.authorizations.is_empty() {
                    let auth_idx = rng.gen_range(0..self.authorizations.len());
                    let (authorized_account, authorization) = &self.authorizations[auth_idx];

                    let calldata = Bytes::from(vec![0u8; 100]); // Simple calldata
                    ...
                    txs.push((tx, *authorized_account));
                } else {
