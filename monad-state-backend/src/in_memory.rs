...
impl InMemoryBlockState {
    pub fn genesis(_: Balance, accounts: BTreeMap<Address, AccountState>) -> Self {
        Self {
...
where
    ...
    SCT: SignatureCollection<NodeIdPubKey = CertificateSignaturePubKey<ST>>,
{
    pub fn genesis(_: Balance, execution_delay: SeqNum) -> InMemoryState<ST, SCT> {
        Arc::new(Mutex::new(Self {
            commits: std::iter::once((
                GENESIS_SEQ_NUM,
                InMemoryBlockState::genesis(Default::default(), BTreeMap::new()),
            ))
            .collect(),
